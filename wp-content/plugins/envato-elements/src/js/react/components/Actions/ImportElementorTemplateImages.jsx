...
const ImportImageInBackground = ({ image, completeCallback }) => {
  const { loading, error, data } = importElementorTemplateImage(image)
...
const ImportElementorTemplateImages = ({ templateKitId, templateId, importAgain, insertToPage, templateData, progressCallback, completeCallback }) => {
        ...
        return (
          <React.Fragment key={`importImage${index}`}>
            {importImageIndex === index || importImageIndex > index ? (
              <ImportImageInBackground key={`importImageBackground${index}`} image={image} completeCallback={importNextImage} />
            ) : null}
          </React.Fragment>
        )
      })}
